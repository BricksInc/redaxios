(function (global, factory) {
	...
	(global = global || self, global.redaxios = factory());
})(this, (function () {
	/**
	...
	function create(defaults) {
	  ...
	  redaxios.get = function (url, config) {
	    return redaxios(url, config, 'get');
	  };

	  /** @public @type {BodylessMethod} */
	  redaxios["delete"] = function (url, config) {
	    return redaxios(url, config, 'delete');
	  };

	  /** @public @type {BodylessMethod} */
	  redaxios.head = function (url, config) {
	    return redaxios(url, config, 'head');
	  };

	  /** @public @type {BodylessMethod} */
	  redaxios.options = function (url, config) {
	    return redaxios(url, config, 'options');
	  };

	  /** @public @type {BodyMethod} */
	  redaxios.post = function (url, data, config) {
	    return redaxios(url, config, 'post', data);
	  };

	  /** @public @type {BodyMethod} */
	  redaxios.put = function (url, data, config) {
	    return redaxios(url, config, 'put', data);
	  };

	  /** @public @type {BodyMethod} */
	  redaxios.patch = function (url, data, config) {
	    return redaxios(url, config, 'patch', data);
	  };
	  ...
	  redaxios.spread = function (fn) {
	    return /** @type {any} */fn.apply.bind(fn, fn);
	  };
	  ...
	  function redaxios(urlOrConfig, config, _method, data, _undefined) {
	    var url = /** @type {string} */typeof urlOrConfig != 'string' ? (config = urlOrConfig).url : urlOrConfig;
	    var response = /** @type {Response<any>} */{
	      config: config
	    };
	    ...
	    return fetchFunc(url, {
	      ...
	      return (isTextLike ? res.text() : res[options.responseType]()).then(function (data) {
	        response.data = data;
	        if (isTextLike) {
	          try {
	        ...
	        var ok = options.validateStatus ? options.validateStatus(res.status) : res.ok;
